...
export function ViewUsers() {
  ...
  async function acceptRequest(u: User): Promise<void> {
    ...
    const requestedBy: string[] = Array.isArray(doctorData.requestedBy) ? doctorData.requestedBy : []
    const newRequestedBy = requestedBy.filter((uid) => uid !== u.id)

    const secretaries: string[] = Array.isArray(doctorData.secretaries) ? doctorData.secretaries : []
    const newSecretaries = [...secretaries, u.id]

    await update(doctorRef, { requestedBy: newRequestedBy, secretaries: newSecretaries })
    ...
    const requestedTo: string[] = Array.isArray(secData.requestedTo) ? secData.requestedTo : []
    const newRequestedTo = requestedTo.filter((uid) => uid !== user.uid)

    const doctors: string[] = Array.isArray(secData.doctors) ? secData.doctors : []
  ...
  return (
    <div className="p-6 space-y-10">
      ...
      <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        {doctors.map((u) =>  { 
          const alreadyLinked = userIsSecretary && user?.doctors?.includes?.(u.id)
          const alreadyRequested = userIsSecretary && user?.requestedTo?.includes?.(u.id)
          return(
            <Card
              key={u.id}
              className="p-4 transition-transform hover:scale-[1.02] bg-blue-500 text-white shadow-lg"
            >
              ...
              <CardContent>
                ...
                <p><strong>Medical ID:</strong> {u.medicalId || "None"}</p>
                <p className="capitalize"><strong>Type:</strong> {u.type}</p>
        ...
        <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
          {secretaries.map((s) => {
            const alreadyLinked = userIsDoctor && user?.secretaries?.includes?.(s.id)
            const hasRequest = userIsDoctor && user?.requestedBy?.includes?.(s.id)
            return(
                <Card
                  key={s.id}
                ...
                >
                  ...
                  <CardContent>
                    ...
                    <p className="capitalize"><strong>Type:</strong> {s.type}</p>
